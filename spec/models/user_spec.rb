...
RSpec.describe User, type: :model do
  ...
  it "is invalid without a role" do
    expect(FactoryGirl.build(:default_user, role: nil)).not_to be_valid
  end
  it "is invalid without a password_confirmation" do
    expect(FactoryGirl.build(:default_user, password_confirmation: nil)).not_to be_valid
  end
  it "is invalid without a matching password and password_confirmation" do
    expect(FactoryGirl.build(:default_user,
                             password: 'aaaaaaaa',
  ...
  it "is an organiser when role is 'Organiser'" do
    expect(FactoryGirl.build(:default_user, role: :Organiser).organiser?).to be_truthy
  end
  it "is not an organiser when role is 'organiser'" do
    expect(FactoryGirl.build(:default_user, role: 'organiser').organiser?).to be_falsey
  end
  it "is not an organiser when role is nil" do
    expect(FactoryGirl.build(:default_user, role: nil).organiser?).to be_falsey
  end

  it "is an overseer when role is 'Overseer'" do
    expect(FactoryGirl.build(:default_user, role: :Overseer).overseer?).to be_truthy
  end
  it "is not an overseer when role is 'overseer'" do
    expect(FactoryGirl.build(:default_user, role: 'overseer').overseer?).to be_falsey
  end
  it "is not an overseer when role is nil" do
    expect(FactoryGirl.build(:default_user, role: nil).overseer?).to be_falsey
  end

  it "is a coordinator when role is 'Coordinator'" do
    expect(FactoryGirl.build(:default_user, role: :Coordinator).coordinator?).to be_truthy
  end
  it "is not a coordinator when role is 'coordinator'" do
    expect(FactoryGirl.build(:default_user, role: 'coordinator').coordinator?).to be_falsey
  end
  it "is not a coordinator when role is nil" do
    expect(FactoryGirl.build(:default_user, role: nil).coordinator?).to be_falsey
  end

  it "is a leader when role is 'Leader'" do
    expect(FactoryGirl.build(:default_user, role: :Leader).leader?).to be_truthy
  end
  it "is not a leader when role is 'leader'" do
    expect(FactoryGirl.build(:default_user, role: 'leader').leader?).to be_falsey
  end
  it "is not a leader when role is nil" do
    expect(FactoryGirl.build(:default_user, role: nil).leader?).to be_falsey
  end
